                ...
                @foreach ($notifications as $notification)
                    <li class="mb-3 p-3 border rounded {{ $notification->read_at ? 'bg-gray-200' : 'bg-blue-100' }}">
                        ...
                        @if (!$notification->read_at)
                            <form action="{{ route('user.notifications.markAsRead', $notification->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="ml-4 text-sm text-gray-600 underline">Marcar como leída</button>
                            </form>
                        @endif
                    </li>
                @endforeach
